...
- An easier way of writing custmoized layers
    - No `initial_weights`, `regularizers`, `constraints` arguments for `__init__`.
...
- numpy, scipy, tqdm, rollbackcontext
...
$ bash dev_scrips/install_dependency
...
For quickly modifying ant test. Run:
~~~{.bash}
$ pyenv activate k2  # or (k3)
$ python setup.py develop  # will link the project root into k2's site package directory
~~~


### Writing Test cases
After implementing some new feature, please write test cases in `tests` directory and run the testing script before sending pull request.
~~~{.bash}
# In project root directory
$ bash dev_scrips/test.sh # testing
~~~

The scripts also check the coding style according to pep8. If something goes wrong, you could use `autopep8` to fix it:
~~~{.bash}
$ autopep8 --recursive -i --select E128 keraflow  # fix all error no.128
~~~

It is highly recommend you avoid these errors when writing them using some editor plugins. If you use vim (or neovim), I recommend installing `flake8` and adopt the settings in this [gist](https://gist.github.com/ipod825/fbee70d8bd063f228951cd4b6f38f4df):
~~~{.bash}
$ pip install flask8
~~~
...
#### No get_config for serialization.
Every layer in Keras has a `get_config` function, which is needed for serializing models. Though its implementation is not necessary for developing algorithms, it would be good for developers to save the time implementing it just for saving their models. 
...
#### Embedding existing layers to write new layers
Currently, in Keras, when writing you own layers, even if you want to conduct similar operation of the `Dense` layer, you still need to define a trainable parameters (write initialization code and add it to the layer's trainable parameters list) for that.

In Keraflow, you could simply write (in `output` function, the correspondence of `get_output_for` in Keras):
~~~{.py}
self.wrap(Dense(output_dim))(input_tensor)
~~~
