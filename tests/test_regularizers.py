...
def test_l1():
    def np_L1(param):
        return np.sum(np.abs(param)) * l_value

    regularizer_test(regularizers.L1(l_value), np_L1, np_input)


def test_l2():
    def np_L2(param):
        return np.sum(np.square(param)) * l_value

    regularizer_test(regularizers.L2(l_value), np_L2, np_input)


def test_l1l2():
    def np_L1L2(param):
        return np.sum(np.abs(param)) * l_value + np.sum(np.square(param)) * l_value

    regularizer_test(regularizers.L1L2(l_value, l_value), np_L1L2, np_input)
